...
async function saveMessage(msg) {
  const db = getDb();
  const now = Date.now();
  const payload = msg.payload ? JSON.stringify(msg.payload) : null;
  await db.execute({
    sql: `INSERT INTO chat_messages (id, thread_id, author, type, text, media_url, lat, lng, reply_to_id, payload, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [
      msg.id,
      msg.threadId,
...
async function listSellerThreads(limit = 15) {
  ...
  const rows = await db.execute({
    sql: `SELECT t.id, t.customer_id, t.needs_seller, t.seller_connected, t.updated_at,
                 ...
                 (SELECT author FROM chat_messages m WHERE m.thread_id = t.id
