const { getDb, uid, normPhone, ensureCustomerSchema, normPhoneE164 } = require('./db');
const { phoneLookupKeys, isCompletePhone, parsePhone } = require('./phone');
...
async function registerCustomer({ name, phone, password }) {
  await ensureCustomerSchema();
  if (!isCompletePhone(phone)) throw new Error('bad_phone');
  const parsed = parsePhone(phone);
  const phoneNorm = parsed.e164 || normPhoneE164(phone) || normPhone(phone);
  ...
  for (const key of keys) {
    const exists = await db.execute({ sql: 'SELECT id FROM customers WHERE phone_norm = ?', args: [key] });
    if (exists.rows.length) return { ok: false, error: 'phone_exists' };
  }
  const id = uid('u');
  const displayPhone = parsed.e164 ? `+${parsed.e164}` : String(phone).trim();
  await db.execute({
    sql: 'INSERT INTO customers (id, name, phone, phone_norm, password_hash, profile) VALUES (?, ?, ?, ?, ?, ?)',
    args: [id, String(name || '').trim(), displayPhone, phoneNorm, hashPassword(password), '{}'],
  });
  return {
    ok: true,
    user: {
...
async function loginCustomer({ phone, password }) {
  ...
  let c = null;
  for (const key of keys) {
    const row = await db.execute({ sql: 'SELECT * FROM customers WHERE phone_norm = ?', args: [key] });
    if (row.rows[0]) {
      c = row.rows[0];
      break;
    }
...
async function updateCustomerProfile(customerId, patch) {
  ...
  if (patch.phone != null && String(patch.phone).trim()) {
    if (!isCompletePhone(patch.phone)) throw new Error('bad_phone');
    const parsed = parsePhone(patch.phone);
    const nextNorm = parsed.e164 || normPhoneE164(patch.phone) || normPhone(patch.phone);
    if (nextNorm.length >= 9 && nextNorm !== String(c.phone_norm || '')) {
      const keys = phoneLookupKeys(patch.phone);
      for (const key of keys) {
      ...
      phone = parsed.e164 ? `+${parsed.e164}` : String(patch.phone).trim();
      phoneNorm = nextNorm;
