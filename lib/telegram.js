const { getSettings, saveSettings } = require('./data');

function publicBase() {
  if (process.env.SITE_URL) return String(process.env.SITE_URL).replace(/\/$/, '');
  if (process.env.VERCEL_URL) return `https://${process.env.VERCEL_URL}`.replace(/\/$/, '');
  return '';
}

function absoluteMediaUrl(url) {
  const u = String(url || '');
  if (!u) return '';
  if (/^https?:\/\//i.test(u)) return u;
  const base = publicBase();
  if (!base) return u;
  return u.startsWith('/') ? base + u : `${base}/${u}`;
}

function escHtml(s) {
  return String(s || '')
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
}

function shortId(id) {
  const s = String(id || '');
  return s.length > 10 ? s.slice(-8) : s;
}

/* ------------------------------------------------------------------
   Access control: owners + allowed users
   ------------------------------------------------------------------ */

function normUser(u) {
  return String(u || '').replace(/^@/, '').trim().toLowerCase();
}

function ownerUsernames() {
  const raw = process.env.TELEGRAM_OWNER_USERS || 'you_can_smilee,kiti1803';
  return raw
    .split(',')
    .map(normUser)
    .filter(Boolean);
}

function isOwnerUsername(u) {
  const n = normUser(u);
  return n ? ownerUsernames().includes(n) : false;
}

function collectSellerChatIds(s) {
  const ids = new Set();
  for (const x of String(process.env.TELEGRAM_SELLER_CHAT_IDS || '').split(',')) {
    const v = String(x || '').trim();
    if (v) ids.add(v);
  }
  if (process.env.TELEGRAM_SELLER_CHAT_ID) ids.add(String(process.env.TELEGRAM_SELLER_CHAT_ID));
  if (s) {
    if (Array.isArray(s.telegramSellers)) {
      for (const t of s.telegramSellers) if (t && t.chatId) ids.add(String(t.chatId));
    }
    if (s.telegramSellerChatId) ids.add(String(s.telegramSellerChatId));
  }
  return Array.from(ids);
}

async function getSellerChatIds() {
  const s = await getSettings().catch(() => null);
  return collectSellerChatIds(s);
}

async function isAllowedTelegramUser(from) {
  if (!from) return false;
  const uname = normUser(from.username);
  if (uname && isOwnerUsername(uname)) return true;
  const s = await getSettings().catch(() => null);
  const allowed = (Array.isArray(s && s.telegramAllowedUsers) ? s.telegramAllowedUsers : []).map(normUser);
  if (uname && allowed.includes(uname)) return true;
  const fromId = String(from.id || '');
  return fromId ? collectSellerChatIds(s).includes(fromId) : false;
}

async function addAllowedUser(username) {
  const uname = normUser(username);
  if (!uname) return { ok: false, error: 'empty' };
  if (isOwnerUsername(uname)) return { ok: false, error: 'owner' };
  const s = (await getSettings()) || {};
  const allowed = (Array.isArray(s.telegramAllowedUsers) ? s.telegramAllowedUsers : []).map(normUser);
  if (allowed.includes(uname)) return { ok: false, error: 'exists' };
  allowed.push(uname);
  await saveSettings({ ...s, telegramAllowedUsers: allowed });
  return { ok: true };
}

async function deleteAllowedUser(username) {
  const uname = normUser(username);
  if (!uname) return { ok: false, error: 'empty' };
  if (isOwnerUsername(uname)) return { ok: false, error: 'owner' };
  const s = (await getSettings()) || {};
  const allowed = (Array.isArray(s.telegramAllowedUsers) ? s.telegramAllowedUsers : []).map(normUser);
  if (!allowed.includes(uname)) return { ok: false, error: 'missing' };
  const sellers = Array.isArray(s.telegramSellers) ? s.telegramSellers : [];
  const removedChatId = (sellers.find((t) => normUser(t && t.username) === uname) || {}).chatId;
  const next = { ...s, telegramAllowedUsers: allowed.filter((u) => u !== uname) };
  next.telegramSellers = sellers.filter((t) => normUser(t && t.username) !== uname);
  if (removedChatId && String(next.telegramSellerChatId || '') === String(removedChatId)) {
    delete next.telegramSellerChatId;
  }
  await saveSettings(next);
  return { ok: true };
}

function customerLabel(customer) {
  return customer?.name || customer?.phone || customer?.email || 'Клиент';
}

function msgBodyPreview(msg) {
  if (msg.type === 'text') return String(msg.text || '').trim() || '—';
  if (msg.type === 'photo') return `📷 Фото${msg.text ? ': ' + msg.text : ''}`;
  if (msg.type === 'voice') return `🎤 Голосовое`;
  if (msg.type === 'location') return `📍 ${msg.lat}, ${msg.lng}`;
  return String(msg.text || msg.type || 'сообщение');
}

function routingTag(threadId, msgId) {
  return `ycs:${threadId}:${msgId}`;
}

function sellerMainKeyboard() {
  return {
    keyboard: [
      [{ text: '📦 Заказы' }, { text: '📋 Чаты' }],
      [{ text: '🔎 Поиск заказа' }, { text: '📞 Найти клиента' }],
      [{ text: '🔌 Отключиться' }, { text: 'ℹ️ Статус' }],
      [{ text: '❓ Помощь' }],
      [{ text: '➕ Добавить пользователя' }, { text: '➖ Удалить пользователя' }],
    ],
    resize_keyboard: true,
    is_persistent: true,
  };
}

function connectKeyboard(threadId) {
  return {
    inline_keyboard: [
      [
        { text: '🟢 Подключиться', callback_data: `connect:${threadId}` },
        { text: '📋 Все чаты', callback_data: 'chats' },
      ],
    ],
  };
}

function chatListKeyboard(threads, activeId) {
  const rows = (threads || []).slice(0, 12).map((t) => {
    const name = t.customer_name || t.customer_phone || shortId(t.id);
    const mark = t.seller_connected || t.id === activeId ? '🟢' : t.needs_seller ? '🟡' : '⚪';
    const label = `${mark} ${String(name).slice(0, 28)}`;
    return [{ text: label, callback_data: `connect:${t.id}` }];
  });
  rows.push([{ text: '🔌 Отключиться', callback_data: 'disconnect' }]);
  return { inline_keyboard: rows };
}

function liveKeyboard(threadId) {
  return {
    inline_keyboard: [
      [
        { text: '🔌 Отключиться', callback_data: 'disconnect' },
        { text: '📋 Чаты', callback_data: 'chats' },
      ],
    ],
  };
}

function orderInfoKeyboard(orderId) {
  return {
    inline_keyboard: [[{ text: '📦 Показать заказ', callback_data: `order:${orderId}` }]],
  };
}

function ordersHubKeyboard(counts) {
  const { ORDER_STATUSES, statusLabelRu } = require(require('path').resolve(process.cwd(), 'lib/orders'));
  const c = counts || {};
  const rows = [];
  for (let i = 0; i < ORDER_STATUSES.length; i += 2) {
    const a = ORDER_STATUSES[i];
    const b = ORDER_STATUSES[i + 1];
    const row = [
      {
        text: `${statusLabelRu(a)} (${c[a] || 0})`,
        callback_data: `orders:${a}`,
      },
    ];
    if (b) {
      row.push({
        text: `${statusLabelRu(b)} (${c[b] || 0})`,
        callback_data: `orders:${b}`,
      });
    }
    rows.push(row);
  }
  rows.push([{ text: '🔎 Поиск по номеру', callback_data: 'orders:search' }]);
  return { inline_keyboard: rows };
}

function ordersListKeyboard(orders, status) {
  const { orderNumber } = require(require('path').resolve(process.cwd(), 'lib/orders'));
  const rows = (orders || []).slice(0, 15).map((o) => {
    const who = (o.customer && (o.customer.contact || o.customer.name || o.customer.phone)) || '—';
    const label = `${orderNumber(o)} · ${String(who).slice(0, 22)}`;
    return [{ text: label, callback_data: `order:${o.id}` }];
  });
  rows.push([{ text: '⬅️ К разделам', callback_data: 'orders:hub' }]);
  if (status) {
    rows.push([{ text: '🔄 Обновить список', callback_data: `orders:${status}` }]);
  }
  return { inline_keyboard: rows };
}

function orderStatusKeyboard(order) {
  const { ORDER_STATUSES, statusLabelRu, normalizeStatus } = require(require('path').resolve(
    process.cwd(),
    'lib/orders'
  ));
  const cur = normalizeStatus(order && order.status);
  const id = order && order.id;
  const rows = [];
  const others = ORDER_STATUSES.filter((s) => s !== cur);
  for (let i = 0; i < others.length; i += 2) {
    const a = others[i];
    const b = others[i + 1];
    const row = [{ text: `→ ${statusLabelRu(a)}`, callback_data: `ostatus:${id}:${a}` }];
    if (b) row.push({ text: `→ ${statusLabelRu(b)}`, callback_data: `ostatus:${id}:${b}` });
    rows.push(row);
  }
  rows.push([
    { text: '📞 Контакты / соцсети', callback_data: `cfind:${id}` },
  ]);
  rows.push([
    { text: '⬅️ К разделу', callback_data: `orders:${cur}` },
    { text: '📦 Разделы', callback_data: 'orders:hub' },
  ]);
  return { inline_keyboard: rows };
}

function prependOrderButton(keyboard, orderId) {
  if (!orderId) return keyboard;
  return {
    inline_keyboard: orderInfoKeyboard(orderId).inline_keyboard.concat(keyboard.inline_keyboard || []),
  };
}

function formatEscalationCard(msg, customer, threadId) {
  const who = escHtml(customerLabel(customer));
  const body = escHtml(msgBodyPreview(msg));
  const tag = routingTag(threadId, msg.id);
  return (
    `🟡 <b>Нужен продавец</b>\n` +
    `👤 <b>${who}</b>\n` +
    `──────────────\n` +
    `${body}\n` +
    `──────────────\n` +
    `<i>Нажмите «Подключиться», чтобы вести диалог.\n` +
    `Или ответьте reply на это сообщение.</i>\n` +
    `<code>${escHtml(tag)}</code>`
  );
}

function formatLiveCard(msg, customer, threadId, opts = {}) {
  const who = escHtml(customerLabel(customer));
  const body = escHtml(msgBodyPreview(msg));
  const tag = routingTag(threadId, msg.id);
  const title = opts.title || '💬 Сообщение клиента';
  return (
    `${title}\n` +
    `👤 <b>${who}</b>\n` +
    `──────────────\n` +
    `${body}\n` +
    `──────────────\n` +
    `<code>${escHtml(tag)}</code>`
  );
}

function formatBotNote(answer, customer, threadId, msgId) {
  const who = escHtml(customerLabel(customer));
  const tag = routingTag(threadId, msgId);
  return (
    `🤖 <b>Бот ответил</b> · ${who}\n` +
    `${escHtml(String(answer || '').slice(0, 500))}\n` +
    `<code>${escHtml(tag)}</code>`
  );
}

async function sendTelegram(payload) {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  if (!token) return null;
  const { chat_id: _omit, method, ...rest } = payload;
  const url = `https://api.telegram.org/bot${token}/${method || 'sendMessage'}`;
  const send = (chatId) =>
    fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ chat_id: chatId, ...rest }),
    })
      .then((r) => r.json())
      .catch(() => null);
  if (payload.chat_id) return send(payload.chat_id);
  const ids = await getSellerChatIds();
  if (!ids.length) return null;
  let last = null;
  for (const id of ids) last = await send(id);
  return last;
}

async function answerCallback(callbackQueryId, text) {
  const token = process.env.TELEGRAM_BOT_TOKEN;
  if (!token || !callbackQueryId) return null;
  return fetch(`https://api.telegram.org/bot${token}/answerCallbackQuery`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      callback_query_id: callbackQueryId,
      text: text || '',
      show_alert: false,
    }),
  }).then((r) => r.json()).catch(() => null);
}

async function getSellerChatId() {
  if (process.env.TELEGRAM_SELLER_CHAT_ID) return process.env.TELEGRAM_SELLER_CHAT_ID;
  try {
    const s = await getSettings();
    return s?.telegramSellerChatId ? String(s.telegramSellerChatId) : null;
  } catch {
    return null;
  }
}

function looksLikeOgg(url) {
  const u = String(url || '').toLowerCase();
  return u.includes('audio/ogg') || u.includes('.ogg') || u.includes('opus');
}

async function sendMediaBundle(msg, captionHtml, replyMarkup) {
  const common = {
    parse_mode: 'HTML',
    reply_markup: replyMarkup,
  };
  if (msg.type === 'photo' && msg.media_url) {
    return sendTelegram({
      method: 'sendPhoto',
      photo: absoluteMediaUrl(msg.media_url),
      caption: captionHtml.slice(0, 1024),
      ...common,
    });
  }
  if (msg.type === 'voice' && msg.media_url) {
    const media = absoluteMediaUrl(msg.media_url);
    if (looksLikeOgg(msg.media_url) || looksLikeOgg(media)) {
      const voiceRes = await sendTelegram({
        method: 'sendVoice',
        voice: media,
        caption: captionHtml.slice(0, 1024),
        ...common,
      });
      if (voiceRes && voiceRes.ok) return voiceRes;
    }
    const audioRes = await sendTelegram({
      method: 'sendAudio',
      audio: media,
      caption: captionHtml.slice(0, 1024),
      ...common,
    });
    if (audioRes && audioRes.ok) return audioRes;
    return sendTelegram({
      method: 'sendDocument',
      document: media,
      caption: captionHtml.slice(0, 1024),
      ...common,
    });
  }
  if (msg.type === 'location') {
    await sendTelegram({
      method: 'sendLocation',
      latitude: Number(msg.lat),
      longitude: Number(msg.lng),
    });
  }
  return sendTelegram({
    method: 'sendMessage',
    text: captionHtml,
    parse_mode: 'HTML',
    reply_markup: replyMarkup,
    disable_web_page_preview: true,
  });
}

/** Live chat while seller is connected (or waiting for seller) */
async function notifySellerLive(msg, customer, threadId, orderId, opts = {}) {
  const text = formatLiveCard(msg, customer, threadId, opts);
  const kb = opts.waiting ? connectKeyboard(threadId) : liveKeyboard(threadId);
  return sendMediaBundle(msg, text, prependOrderButton(kb, orderId));
}

function isOrderStatusMessage(m) {
  if (m && m.payload && m.payload.kind === 'order_status') return true;
  const t = String((m && m.text) || '');
  return /статус\s*«/.test(t) && /order-status\.html/i.test(t);
}

function msgPreviewLine(m) {
  if (!m) return '—';
  if (m.type === 'photo') return '📷 фото';
  if (m.type === 'voice') return '🎤 голос';
  if (m.type === 'location') return '📍 локация';
  if (m.type === 'product') return String(m.text || '🛍 товар').replace(/\s+/g, ' ').slice(0, 160);
  return String(m.text || m.type || '—').replace(/\s+/g, ' ').slice(0, 220);
}

/**
 * History dump for Telegram when seller connects to a site chat.
 * Customer lines first (what they asked), then short context — skips order-status spam.
 */
function formatConnectHistory(messages) {
  const all = Array.isArray(messages) ? messages : [];
  const useful = all.filter((m) => !isOrderStatusMessage(m));
  const customerMsgs = useful.filter((m) => m.author === 'customer');
  const lines = [];

  if (customerMsgs.length) {
    lines.push('<b>💬 Что писал клиент:</b>');
    const recentCust = customerMsgs.slice(-20);
    recentCust.forEach((m, i) => {
      lines.push(`${i + 1}. ${escHtml(msgPreviewLine(m))}`);
    });
    if (customerMsgs.length > recentCust.length) {
      lines.push(`<i>…ещё ${customerMsgs.length - recentCust.length} ранее</i>`);
    }
  } else {
    lines.push('<b>💬 Что писал клиент:</b>');
    lines.push('<i>Пока нет сообщений от клиента в чате на сайте.</i>');
  }

  const context = useful.slice(-10);
  if (context.length) {
    lines.push('');
    lines.push('<b>📋 Последний диалог:</b>');
    context.forEach((m) => {
      const who =
        m.author === 'customer' ? '👤 клиент' : m.author === 'seller' ? '🧑‍💼 вы' : '🤖 бот';
      lines.push(`${who}: ${escHtml(msgPreviewLine(m))}`);
    });
  }

  let text = lines.join('\n');
  if (text.length > 3500) text = text.slice(0, 3490) + '…';
  return text;
}

function chatListPreview(t) {
  const raw = String(t.last_text || t.last_type || '—').replace(/\s+/g, ' ').trim();
  const author = String(t.last_author || '');
  const prefix =
    author === 'customer' ? '👤 ' : author === 'seller' ? '🧑‍💼 ' : author === 'agent' ? '🤖 ' : '';
  return `${prefix}${raw}`.slice(0, 48);
}

/** Alert when bot cannot answer and seller is not connected */
async function notifySellerRequest(msg, customer, threadId, orderId) {
  const text = formatEscalationCard(msg, customer, threadId);
  return sendMediaBundle(msg, text, prependOrderButton(connectKeyboard(threadId), orderId));
}

/** Optional note that bot answered (only when seller connected) */
async function notifySellerBotAnswer(answer, customer, threadId, msgId) {
  return sendTelegram({
    method: 'sendMessage',
    text: formatBotNote(answer, customer, threadId, msgId),
    parse_mode: 'HTML',
    disable_web_page_preview: true,
  });
}

function starsLine(rating) {
  const r = Math.min(5, Math.max(1, Number(rating) || 0));
  return '⭐'.repeat(r) + '☆'.repeat(5 - r);
}

function formatAiReportCard({ customer, rating, comment, connected, question }) {
  const who = escHtml(customerLabel(customer));
  const q = String(question || '').trim();
  const cmt = String(comment || '').trim();
  const low = Number(rating) <= 3;
  const lines = [];
  lines.push(low ? '⚠️ <b>Чат завершён клиентом · низкая оценка</b>' : '⭐ <b>Чат завершён клиентом</b>');
  lines.push(`👤 <b>${who}</b>`);
  lines.push('──────────────');
  lines.push(`🤖 Оценка работы ИИ: <b>${starsLine(rating)} (${rating}/5)</b>`);
  lines.push(`👤 Вёл диалог: ${connected ? 'продавец' : 'ИИ-ассистент'}`);
  if (q) lines.push(`💬 Вопрос: ${escHtml(q.slice(0, 160))}`);
  if (cmt) lines.push(`📝 Комментарий: ${escHtml(cmt.slice(0, 400))}`);
  lines.push('──────────────');
  lines.push('<i>Если клиенту нужна помощь — нажмите «Связаться».</i>');
  return lines.join('\n');
}

/** Mini-report to the seller when a customer closes the chat with a rating */
async function notifySellerAiReport({ customer, threadId, rating, comment, connected, question, orderId }) {
  const text = formatAiReportCard({ customer, rating, comment, connected, question });
  let kb = connectKeyboard(threadId);
  if (Number(rating) <= 3) {
    kb = {
      inline_keyboard: [{ text: '🟢 Связаться', callback_data: `connect:${threadId}` }].concat(
        kb.inline_keyboard || []
      ),
    };
  }
  return sendTelegram({
    method: 'sendMessage',
    text,
    parse_mode: 'HTML',
    disable_web_page_preview: true,
    reply_markup: prependOrderButton(kb, orderId),
  });
}

async function sendSellerHelp(chatId) {
  return sendTelegram({
    chat_id: chatId,
    method: 'sendMessage',
    parse_mode: 'HTML',
    reply_markup: sellerMainKeyboard(),
    text:
      `<b>YouCanSmile — кабинет продавца</b>\n\n` +
      `🤖 Бот сам отвечает на базовые вопросы (доставка, оплата, самовывоз…).\n` +
      `Вам приходят только диалоги, где нужна помощь.\n\n` +
      `<b>Как работать</b>\n` +
      `1. «📦 Заказы» — разделы по статусу, смена статуса\n` +
      `2. «🔎 Поиск заказа» — найти по 6-значному номеру\n` +
      `3. «📋 Чаты» — список клиентов → «Подключиться»\n` +
      `4. «📞 Найти клиента» — телефон → Telegram / Instagram / WhatsApp\n` +
      `5. Пишите обычные сообщения — они уйдут клиенту на сайт\n` +
      `6. «🔌 Отключиться» — выйти из диалога\n` +
      `7. «➕/➖ Пользователи» — управление доступом к боту (только владельцы)\n\n` +
      `Команды: /orders /chats /findclient /status /disconnect /help /adduser /deluser`,
  });
}

async function registerSellerChat(chatId, username) {
  const id = String(chatId);
  try {
    const s = (await getSettings()) || {};
    const sellers = Array.isArray(s.telegramSellers) ? s.telegramSellers : [];
    const existing = sellers.find((t) => t && String(t.chatId) === id);
    const uname = normUser(username);
    if (existing) {
      existing.username = uname || existing.username;
      existing.lastSeen = Date.now();
    } else {
      sellers.push({ chatId: id, username: uname, addedAt: Date.now(), lastSeen: Date.now() });
    }
    await saveSettings({ ...s, telegramSellers: sellers, telegramSellerChatId: id });
  } catch {
    /* ignore */
  }
}

/** @deprecated keep for older call sites */
async function notifySellerMessage(msg, customer, threadId) {
  return notifySellerLive(msg, customer, threadId);
}

function formatMsgForTelegram(msg, customer, threadId) {
  return formatLiveCard(msg, customer, threadId).replace(/<[^>]+>/g, '');
}

/* ------------------------------------------------------------------
   New order notifications to the seller
   ------------------------------------------------------------------ */

function txt(v) {
  if (v == null) return '';
  if (typeof v === 'string') return v;
  if (typeof v === 'object') return v.ru || v.uz || v.en || Object.values(v)[0] || '';
  return String(v);
}

/** Persist a data: URL into the media table and return a fetchable URL. */
async function persistDataImage(dataUrl) {
  const raw = String(dataUrl || '');
  if (!raw) return '';
  if (!raw.startsWith('data:')) return raw; // already a URL
  const m = raw.match(/^data:([^;,]+);base64,(.+)$/);
  if (!m) return '';
  const { saveMedia } = require('./chat');
  const { uid } = require('./db');
  const id = uid('md');
  await saveMedia({ id, mime: m[1] || 'application/octet-stream', data: raw });
  return `/api/media?id=${id}`;
}

async function orderCardLines(order) {
  const c = order.customer || {};
  const isCustom = order.type === 'custom';
  const lines = [];
  try {
    const { orderNumberLabel } = require('./orders');
    lines.push(`📦 <b>${escHtml(orderNumberLabel(order, 'ru'))}</b>`);
  } catch (_) {
    lines.push(`📦 <b>${escHtml(order.id)}</b>`);
  }
  if (order.createdAt) {
    const d = new Date(order.createdAt);
    lines.push(
      `🕒 ${d.toLocaleString('ru-RU', {
        timeZone: 'Asia/Tashkent',
        day: '2-digit',
        month: '2-digit',
        year: 'numeric',
        hour: '2-digit',
        minute: '2-digit',
        second: '2-digit',
      })}`
    );
  }

  if (isCustom) {
    const desc = String(order.customDescription || c.note || '').trim();
    if (desc) lines.push(`📝 ${escHtml(desc)}`);
  } else {
    const items = (order.items || [])
      .map((i) => `• ${escHtml(i.title)} × ${i.qty} = ${i.price * i.qty}`)
      .join('\n');
    if (items) lines.push(`🧾 ${items}`);
    if (order.total != null) {
      lines.push(`💰 Итого: ${order.total}${order.currency ? ' ' + escHtml(order.currency) : ''}`);
    }
  }

  lines.push('──────────────');
  if (c.name) lines.push(`👤 <b>${escHtml(c.name)}</b>`);
  if (c.phone) lines.push(`📞 ${escHtml(c.phone)}`);
  if (c.telegramVerified || c.telegramId) {
    const un = c.telegramUsername || c.contact || '';
    lines.push(`✈️ Telegram ✅ ${un ? escHtml(un.startsWith('@') ? un : '@' + un) : '<i>подтверждён</i>'}`);
  } else if (c.contact) {
    const ch = c.contactChannel === 'instagram' ? 'Instagram' : c.contactChannel === 'telegram' ? 'Telegram' : '';
    lines.push(`💬 ${ch ? ch + ': ' : ''}${escHtml(c.contact)}`);
  }
  if (order.fulfillment) {
    const f =
      order.fulfillment === 'pickup'
        ? 'Самовывоз'
        : order.fulfillment === 'delivery'
          ? 'Доставка'
          : order.fulfillment;
    lines.push(`📦 ${escHtml(f)}`);
    if (order.fulfillment === 'pickup' && order.pickupPoint) {
      try {
        const s = (await getSettings()) || {};
        const p = ((s.pickupPoints || [])).find((x) => x.id === order.pickupPoint);
        lines.push(`📍 ${escHtml(p ? txt(p.name) || txt(p.address) || order.pickupPoint : order.pickupPoint)}`);
      } catch {
        /* ignore */
      }
    }
  }
  if (c.address) lines.push(`📍 ${escHtml(c.address)}`);
  if (Array.isArray(order.coords) && order.coords.length === 2) {
    lines.push(`🧭 ${escHtml(order.coords.join(', '))}`);
  }
  if (order.payment) {
    const pay = order.payment === 'card' ? 'Карта' : order.payment === 'cash' ? 'Наличные' : order.payment;
    lines.push(`💳 ${escHtml(pay)}`);
  }
  return lines;
}

async function formatOrderCard(order) {
  const isCustom = order.type === 'custom';
  const lines = [
    isCustom ? '🔥 <b>Новый индивидуальный заказ</b>' : '🛍️ <b>Новый заказ</b>',
    '──────────────',
  ];
  lines.push(...(await orderCardLines(order)));
  lines.push(`\n<i>Продавец свяжется с клиентом для уточнения деталей.</i>`);
  return lines.join('\n');
}

async function formatOrderDetails(order) {
  const { statusLabelRu } = require('./orders');
  const lines = ['📦 <b>Заказ клиента</b>', '──────────────'];
  lines.push(`📌 Статус: <b>${escHtml(statusLabelRu(order.status))}</b>`);
  lines.push(...(await orderCardLines(order)));
  return lines.join('\n');
}

function socialHref(kind, value) {
  const v = String(value || '').trim().replace(/^@+/, '');
  if (!v) return '';
  if (/^https?:\/\//i.test(v)) return v;
  if (kind === 'telegram') return `https://t.me/${encodeURIComponent(v)}`;
  if (kind === 'instagram') return `https://instagram.com/${encodeURIComponent(v)}`;
  return '';
}

function contactLookupKeyboard(result) {
  const rows = [];
  const intl = result && result.intl ? String(result.intl) : '';
  const linkRow = [];
  if (intl) {
    linkRow.push({ text: '💬 WhatsApp', url: `https://wa.me/${intl}` });
    linkRow.push({ text: '✈️ Telegram по номеру', url: `https://t.me/+${intl}` });
  }
  if (linkRow.length) rows.push(linkRow);

  const seen = new Set();
  const addUserLink = (label, url) => {
    if (!url || seen.has(url)) return;
    seen.add(url);
    rows.push([{ text: label, url }]);
  };

  for (const a of (result && result.accounts) || []) {
    if (a.telegram) addUserLink(`✈️ TG @${String(a.telegram).replace(/^@/, '')}`, socialHref('telegram', a.telegram));
    if (a.instagram) addUserLink(`📷 IG @${String(a.instagram).replace(/^@/, '')}`, socialHref('instagram', a.instagram));
  }
  for (const oc of (result && result.orderContacts) || []) {
    const handle = String(oc.contact || '')
      .trim()
      .replace(/^@+/, '')
      .replace(/^https?:\/\/(www\.)?(t\.me|telegram\.me)\//i, '')
      .replace(/^https?:\/\/(www\.)?instagram\.com\//i, '')
      .replace(/\/$/, '');
    if (!handle) continue;
    if (oc.channel === 'instagram') {
      addUserLink(`📷 IG @${handle}`, socialHref('instagram', handle));
    } else if (oc.channel === 'telegram') {
      addUserLink(`✈️ TG @${handle}`, socialHref('telegram', handle));
    } else if (/insta/i.test(String(oc.contact || ''))) {
      addUserLink(`📷 IG @${handle}`, socialHref('instagram', handle));
    } else {
      addUserLink(`💬 ${handle}`, socialHref('telegram', handle));
    }
  }

  rows.push([{ text: '📞 Другой номер', callback_data: 'cfind:ask' }]);
  rows.push([{ text: '📦 Заказы', callback_data: 'orders:hub' }, { text: '📋 Чаты', callback_data: 'chats' }]);
  return { inline_keyboard: rows.slice(0, 12) };
}

function formatContactLookupCard(result) {
  if (!result || !result.ok) {
    return 'Не удалось разобрать номер. Пришлите телефон целиком, например <code>+79830646154</code>.';
  }
  const lines = [];
  lines.push('📞 <b>Поиск клиента</b>');
  lines.push(`Запрос: <code>${escHtml(result.query)}</code>`);
  if (result.intl) lines.push(`Нормализован: <code>+${escHtml(result.intl)}</code>`);
  lines.push('──────────────');

  const accounts = result.accounts || [];
  const orders = result.orders || [];
  const orderContacts = result.orderContacts || [];

  if (!accounts.length && !orders.length) {
    lines.push('В базе YouCanSmile по этому номеру <b>ничего не найдено</b>.');
    lines.push('');
    lines.push('Можно всё равно попробовать написать:');
    if (result.intl) {
      lines.push(`• WhatsApp: https://wa.me/${escHtml(result.intl)}`);
      lines.push(`• Telegram по номеру: https://t.me/+${escHtml(result.intl)}`);
    }
    lines.push('');
    lines.push('<i>Telegram / Instagram откроются только если клиент разрешил поиск по номеру или указал @username при заказе.</i>');
    return lines.join('\n');
  }

  if (accounts.length) {
    lines.push('<b>👤 Аккаунт на сайте</b>');
    for (const a of accounts) {
      lines.push(`• ${escHtml(a.name || 'без имени')} · ${escHtml(a.phone || '—')}`);
      if (a.telegram) lines.push(`  ✈️ Telegram: @${escHtml(String(a.telegram).replace(/^@/, ''))}`);
      else lines.push('  ✈️ Telegram: <i>не указан</i>');
      if (a.instagram) lines.push(`  📷 Instagram: @${escHtml(String(a.instagram).replace(/^@/, ''))}`);
      else lines.push('  📷 Instagram: <i>не указан</i>');
      if (a.email) lines.push(`  ✉️ ${escHtml(a.email)}`);
    }
    lines.push('');
  }

  if (orderContacts.length) {
    lines.push('<b>💬 Контакт из заказов</b>');
    const uniq = new Map();
    for (const oc of orderContacts) {
      const key = `${oc.channel}|${oc.contact}`.toLowerCase();
      if (!oc.contact || uniq.has(key)) continue;
      uniq.set(key, oc);
    }
    for (const oc of uniq.values()) {
      const ch =
        oc.channel === 'instagram' ? 'Instagram' : oc.channel === 'telegram' ? 'Telegram' : oc.channel || 'контакт';
      lines.push(`• ${escHtml(ch)}: <b>${escHtml(oc.contact)}</b>`);
    }
    lines.push('');
  } else if (orders.length) {
    lines.push('<b>💬 Контакт из заказов:</b> <i>не заполнен</i> (только телефон)');
    lines.push('');
  }

  if (orders.length) {
    const { orderNumber, statusLabelRu } = require('./orders');
    lines.push(`<b>📦 Заказы (${orders.length})</b>`);
    for (const o of orders.slice(0, 8)) {
      const who = (o.customer && o.customer.name) || '—';
      lines.push(
        `• #${escHtml(orderNumber(o))} · ${escHtml(statusLabelRu(o.status))} · ${escHtml(String(who).slice(0, 28))}`
      );
    }
    lines.push('');
  }

  lines.push('<b>Быстрые ссылки</b> — кнопки ниже.');
  lines.push('<i>Если соцсети пустые: клиент не указал их при заказе/в профиле. Кнопки WhatsApp и Telegram по номеру всё равно могут сработать.</i>');
  return lines.join('\n');
}

/** Notify the seller about a new order (with reference / receipt photo when present). */
async function notifySellerNewOrder(order) {
  const caption = await formatOrderCard(order);
  const replyMarkup = orderStatusKeyboard(order);
  let photoUrl = '';
  if (order.type === 'custom' && order.characterImage) {
    photoUrl = await persistDataImage(order.characterImage);
  } else if (order.paymentReceipt) {
    photoUrl = await persistDataImage(order.paymentReceipt);
  }
  if (photoUrl) {
    return sendTelegram({
      method: 'sendPhoto',
      photo: absoluteMediaUrl(photoUrl),
      caption: caption.slice(0, 1024),
      parse_mode: 'HTML',
      disable_web_page_preview: true,
      reply_markup: replyMarkup,
    });
  }
  return sendTelegram({
    method: 'sendMessage',
    text: caption,
    parse_mode: 'HTML',
    disable_web_page_preview: true,
    reply_markup: replyMarkup,
  });
}

module.exports = {
  getSettings,
  sendTelegram,
  answerCallback,
  notifySellerMessage,
  notifySellerLive,
  notifySellerRequest,
  notifySellerBotAnswer,
  notifySellerAiReport,
  formatMsgForTelegram,
  formatEscalationCard,
  formatLiveCard,
  registerSellerChat,
  getSellerChatId,
  getSellerChatIds,
  isAllowedTelegramUser,
  isOwnerUsername,
  normUser,
  ownerUsernames,
  addAllowedUser,
  deleteAllowedUser,
  absoluteMediaUrl,
  publicBase,
  sellerMainKeyboard,
  connectKeyboard,
  chatListKeyboard,
  liveKeyboard,
  orderInfoKeyboard,
  ordersHubKeyboard,
  ordersListKeyboard,
  orderStatusKeyboard,
  formatOrderDetails,
  sendSellerHelp,
  routingTag,
  customerLabel,
  msgBodyPreview,
  escHtml,
  shortId,
  notifySellerNewOrder,
  formatConnectHistory,
  isOrderStatusMessage,
  chatListPreview,
  formatContactLookupCard,
  contactLookupKeyboard,
  formatAiReportCard,
  starsLine,
};
