...
const PRODUCT_INTENT_RE =
  /в наличии|наличие|есть ли|есть(?!\p{L})|сколько стоит|цена|ценник|доступн|купить|куплю|заказать|хочу(?!\p{L})|ищу|нужн|посовету|подскажи|покажи|какие есть|что есть|каталог|ассортимент/u;

const EXPLICIT_PRODUCT_RE =
  /медуз|смайл|smile|crystal|кристалл|sage|брелок|брелк|кулон|чокер|браслет|подвеск|бусин|ключниц|украшени|кольц|серьг|цепочк|band|колье/;
...
function scoreProducts(q, products, categories) {
  ...
  const qWords = wordsOf(nq);
  if (!qWords.length) return [];

  const scored = (products || []).map((p) => {
    let score = 0;
    for (const tw of wordsOf(txt(p.title))) {
      let best = 0;
      for (const qw of qWords) best = Math.max(best, sharedPrefixLen(tw, qw));
      if (best >= 4) score += best;
    }
    const cat = catNameOf(p, categories);
    if (cat) {
      for (const cw of wordsOf(cat)) {
        let best = 0;
        for (const qw of qWords) best = Math.max(best, sharedPrefixLen(cw, qw));
        if (best >= 4) score += Math.round(best * 0.6);
      }
    }
    return { p, score };
  });
...
function buildSystemPrompt(knowledgeBase) {
  return (
    ...
    'РОЛЬ ПАССИВНОГО ПРОДАВЦА:\n' +
    '- Будь дружелюбным, но ненавязчивым, как хороший консультант в магазине.\n' +
    '- Если уместно, после ответа задай ОДИН уточняющий вопрос (сколько штук нужно, оформить сразу, показать похожие). Не задавай больше одного вопроса в сообщении.\n\n' +
    knowledgeBase
  );
}
...
async function geminiReply(text, orders, settings, opts = {}) {
  ...
  const attempt = async () => {
    ...
    if (/ESCALATE/i.test(answer)) return { answer: null, escalate: true, retryable: false };
    return { answer, escalate: false, retryable: false };
  };
  ...
  for (let i = 0; i < 2; i++) {
    try {
      ...
      if (!r.retryable || i === 1) return { answer: null, escalate: true };
      await sleep(300);
    } catch (_) {
      if (i === 1) return { answer: null, escalate: true };
      await sleep(300);
    }
  }
...
async function aiReply(text, orders, settings, opts = {}) {
  ...
  const q = String(text || '').toLowerCase();
  const productIntent = PRODUCT_INTENT_RE.test(q);

  if (productIntent) {
    const match = buildProductAnswer(q, products, categories);
    if (match) return match;
    if (EXPLICIT_PRODUCT_RE.test(q)) {
      const lang = detectLang(q);
      return { answer: NO_SUCH[lang] || NO_SUCH.ru, escalate: false };
    }
  }

  return geminiReply(text, orders, settings, { ...opts, products, categories });
}
