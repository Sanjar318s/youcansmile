...
const FAQ_PICKUP_RE =
  /самовывоз|время\s*самовывоз|где\s*.*(магазин|точка|забрать)|куда\s*(прийти|подъехать|забрать)|точка\s*самовывоз|забрать\s*(заказ|у\s*вас)?|pickup|olib\s*ketish/i;
const FAQ_PICKUP_TIME_RE =
  /во\s*скольк\w*|до\s*скольк\w*|со\s*скольк\w*|время\s*(работы|открыт|забор|самовывоз)|когда\s+(можно\s+)?(забрать|прийти|подъехать)|час(ы|ов)?\s*(работы|открыт)|график\s*работы|working\s*hours|qachon\s+(olib|kel)/i;
...
function buildSystemPrompt(knowledgeBase) {
  return (
    ...
    'СТРОГИЕ ПРАВИЛА:\n' +
    '- Отвечай ТОЛЬКО по данным ниже. Не выдумывай цены, наличие, адреса, акции, часы работы.\n' +
    ...
    '- Если клиент уточняет тот же вопрос (например время самовывоза), а в данных нет ответа — пиши ESCALATE. Не уходи в каталог товаров.\n' +
    '- Если точного товара нет — скажи об этом и предложи похожее из каталога или «что есть в наличии».\n' +
    '- ESCALATE пиши если: клиент ЯВНО просит продавца/оператора; спор/возврат/жалоба; или в данных НЕТ ответа на сервисный вопрос (часы самовывоза, точная сумма и т.п.) и уточнение уже было.\n' +
    '- Не пиши ESCALATE из‑за неуверенности по товарам — дай лучший ответ по каталогу.\n' +
...
function recentTopicPickup(history) {
  const recent = (Array.isArray(history) ? history : []).slice(-10);
  return recent.some((m) =>
    /самовывоз|pickup|olib\s*ketish|точка\s*самовывоз|парус|забрать|прийти\s*за|адрес\s*самовывоз/i.test(
      String(m.text || '')
    )
  );
}

function vagueTimeQuestion(q) {
  const s = String(q || '').trim();
  return (
    /^\s*во\s*скольк\p{L}*\s*\??\s*$/iu.test(s) ||
    /^\s*а\s+во\s*скольк\p{L}*\s*\??\s*$/iu.test(s) ||
    /^\s*когда\s*\??\s*$/iu.test(s) ||
    /^\s*время\s*\??\s*$/iu.test(s)
  );
}

/** Hours/schedule text from settings, if any. Pickup points only store address today. */
function pickupHoursFromSettings(s) {
  const chunks = [
    txt(s.shipping_pickup_text),
    txt(s.shippingPickupText),
    txt(s.pickup_hours),
    txt(s.pickupHours),
    txt(s.work_hours),
    txt(s.workHours),
    ...
    .map((x) => String(x || '').trim())
    .filter(Boolean);
  const withTime = chunks.find((c) =>
    /\d{1,2}\s*[:.]\s*\d{2}|\d{1,2}\s*[–—-]\s*\d{1,2}|час|с\s*\d|до\s*\d|открыт|работаем|график/i.test(c)
  );
  return withTime || null;
}

function wantsPickupTime(q, history) {
  const text = stripChatNoise(q);
  if (FAQ_PICKUP_TIME_RE.test(text) && (FAQ_PICKUP_RE.test(text) || recentTopicPickup(history) || /забрать|самовывоз|pickup|прийти/i.test(text))) {
    return true;
  }
  if (FAQ_PICKUP_TIME_RE.test(text) && /забрать|самовывоз|pickup|точка|парус|прийти|подъехать/i.test(text)) {
    return true;
  }
  // "Во сколько можно забрать" / follow-up after we already talked pickup
  if (recentTopicPickup(history) && (FAQ_PICKUP_TIME_RE.test(text) || vagueTimeQuestion(text) || /можно\s+забрать|когда\s+забрать/i.test(text))) {
    return true;
  }
  return false;
}

function isServiceOrFaqTopic(text, history) {
  const q = stripChatNoise(text);
  if (
    FAQ_DELIVERY_RE.test(q) ||
    FAQ_PICKUP_RE.test(q) ||
    FAQ_PAY_RE.test(q) ||
    FAQ_LEADTIME_RE.test(q) ||
    FAQ_CUSTOM_RE.test(q) ||
    CALL_SELLER_RE.test(q)
  ) {
    return true;
  }
  // Time questions only count as service if tied to pickup/delivery context
  if (wantsPickupTime(q, history)) return true;
  if (FAQ_PICKUP_TIME_RE.test(q) && recentTopicDelivery(history)) return true;
  if (recentTopicPickup(history) && (vagueTimeQuestion(q) || /забрать|прийти|адрес|точка|время|час/i.test(q))) {
    return true;
  }
  if (recentTopicDelivery(history) && (vagueTimeQuestion(q) || /скольк|цена|стоим|когда/i.test(q))) {
    return true;
  }
  return false;
}
...
function faqReply(text, settings, history) {
  ...
  // Vague "во сколько?" — clarify once; if already on pickup topic → hours path
  if (vagueTimeQuestion(q) && !recentTopicPickup(history) && !recentTopicDelivery(history)) {
    const ans =
      lang === 'uz'
        ? "Nimani nazarda tutyapsiz: olib ketish vaqti yoki yetkazib berish?"
        : lang === 'en'
          ? 'Do you mean pickup hours or delivery time?'
          : 'Уточните, пожалуйста: время самовывоза или время доставки?';
    return { answer: ans, escalate: false };
  }
  ...
  // Pickup hours — if we don't have schedule in settings, escalate to seller (don't derail to catalog)
  if (wantsPickupTime(q, history)) {
    const hours = pickupHoursFromSettings(s);
    if (hours) {
      const head =
        lang === 'uz' ? 'Olib ketish vaqti:\n' : lang === 'en' ? 'Pickup hours:\n' : 'Время самовывоза:\n';
      const loc = pickup.length
        ? `\n${lang === 'en' ? 'Points:' : lang === 'uz' ? 'Nuqtalar:' : 'Точки:'}\n${pickup.join('\n')}`
        : '';
      return { answer: head + hours + loc, escalate: false };
    }
    return { answer: null, escalate: true };
  }

  if (FAQ_PICKUP_RE.test(q) || (recentTopicPickup(history) && /адрес|где|точка|куда/i.test(q))) {
    if (pickup.length) {
      const head =
        lang === 'uz' ? "Olib ketish nuqtalari:\n" : lang === 'en' ? 'Pickup points:\n' : 'Точки самовывоза:\n';
      const hint =
        lang === 'uz'
          ? '\nVaqt (soat) haqida aniq ma’lumot yo‘q — yozing «позвать продавца».'
          : lang === 'en'
            ? '\nExact opening hours aren’t in our data — type “call seller” if you need them.'
            : '\nТочное время работы в данных нет — если нужно узнать часы, напишите «позвать продавца».';
...
async function aiReply(text, orders, settings, opts = {}) {
  ...
  if (faq && (faq.answer || faq.escalate)) return faq;

  // Service/FAQ follow-ups must never fall through to random product cards
  if (isServiceOrFaqTopic(raw, history)) {
    const again = faqReply(raw, settings, history);
    if (again && (again.answer || again.escalate)) return again;
    // No data for this service question → seller
    return { answer: null, escalate: true };
  }
  ...
  if (gem.escalate) {
    if (
      CALL_SELLER_RE.test(raw) ||
      /жалоб|возврат|обман|претенз|refund|complaint/i.test(raw) ||
      isServiceOrFaqTopic(raw, history)
    ) {
      return { answer: null, escalate: true };
    }
    return autoFallback(raw, products, categories, history);
