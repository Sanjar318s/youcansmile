/* ============================================================
   YouCanSmile — public order status page (live refresh)
   ============================================================ */
(async function initOrderStatus() {
  ...
  function orderFingerprint(order) {
    if (!order) return '';
    return `${order.id}:${normalizeStatus(order.status)}:${order.updatedAt || order.createdAt || 0}:${order.total || 0}`;
  }
  ...
  function renderCard(order) {
    ...
    card.innerHTML = `
      ...
      <div class="os-meta">
        ...
        ${order.total != null ? `<div><b>${I18n.t('cart_total')}:</b> ${Store.formatPrice(order.total, settings)}</div>` : ''}
      </div>
  ...
  async function refresh(force) {
    ...
    if (!force && orderFingerprint(order) === orderFingerprint(currentOrder)) return;
    renderCard(order);
  }
